...
namespace SMGI.Plugin.MapGeneralization.Algrithm
{
    //地理特征保持的弯曲化简
    public class SimplifyByDTCharacters
    {
        //特征点
        public static IPointCollection CharacterPoint = new MultipointClass();
        enum NodeType
        {
            Super = int.MaxValue
        }
        ...
        static IPath RemovePoints(IPath p, List<ITinEdge> removedBends)
        {
            ...
            //获取移除的bend在曲线中的那一段
            List<IPolyline> bendLine = new List<IPolyline>();
            IGeometryCollection polyline = new PolylineClass();
            polyline.AddGeometry(p);
           
            for (int bend = 0; bend < removedBends.Count; bend++)
            {
                #region
                //截取线段
                IPoint fpoint = new PointClass();
                removedBends[bend].FromNode.QueryAsPoint(fpoint);
                IPoint tpoint = new PointClass();
                IPoint outpt = new PointClass();
                removedBends[bend].ToNode.QueryAsPoint(tpoint);
                double fdis = 0;
                double dis = 0;
                double dis1 = 0;
                bool flag = true;
                (polyline as IPolycurve).QueryPointAndDistance(esriSegmentExtension.esriNoExtension, fpoint, false, outpt, ref dis, ref fdis, flag);
                (polyline as IPolycurve).QueryPointAndDistance(esriSegmentExtension.esriNoExtension, tpoint, false, outpt, ref dis1, ref fdis, flag);
                ICurve outCurve = new PolylineClass();
                (polyline as IPolycurve).GetSubcurve(dis, dis1, false, out outCurve);

                PathClass removepath = new PathClass();
               
                removepath.AddPointCollection(outCurve as IPointCollection);
               
                IGeometryCollection remove = new PolylineClass();
                remove.AddGeometry(removepath as IGeometry);
                bendLine.Add(remove as IPolyline);
                #endregion
            }
            int curEdgeIndex = 0;
            ...
            for (int i = 0; i < pc.PointCount; i++)
            {
                ...
                {
                    //移除的bend是否存在特征点
                    var re = bendLine[curEdgeIndex] as IRelationalOperator;
                    Dictionary<IPoint, double> pointDis = new Dictionary<IPoint, double>();
                    for (int pb = 0; pb < CharacterPoint.PointCount; pb++)
                    {
                        var pt = CharacterPoint.get_Point(pb);
                        if (!re.Disjoint(pt as IGeometry))
                        {
                            double fdis = 0;
                            double dis = 0;
                            bool flag = true;
                            IPoint outpt = new PointClass();
                            (polyline as IPolycurve).QueryPointAndDistance(esriSegmentExtension.esriNoExtension, pt, false, outpt, ref dis, ref fdis, flag);
                            pointDis[pt] = dis;

                        }

                    }
                    var sortpt = pointDis.OrderBy(t => t.Value);
                    foreach (var kv in sortpt)
                    {
                        r.AddPoint(kv.Key);
                    }
                    r.AddPoint(pc.get_Point(i));
