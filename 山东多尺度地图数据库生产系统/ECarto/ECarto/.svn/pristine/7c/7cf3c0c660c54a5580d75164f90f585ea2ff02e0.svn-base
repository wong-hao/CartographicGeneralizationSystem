...
{
    public class SubmitCommand : SMGICommand
    {
        ...
        //提交服务器端数据库(需同时更新服务器端数据与本地数据库数据，保持提交后本地数据库的数据与服务器端数据一致)
        public override void OnClick()
        {
            ...
            if (DialogResult.OK == submitDlg.ShowDialog())
            {
                ...
                using (var wo = GApplication.Application.SetBusy())
                {
                    ...
                    int serverLatestVersion = ds.UpdateDatabase(fcName2editedFeatures, localDT, submitDlg.SubmitDesc, System.Environment.MachineName, wo);
