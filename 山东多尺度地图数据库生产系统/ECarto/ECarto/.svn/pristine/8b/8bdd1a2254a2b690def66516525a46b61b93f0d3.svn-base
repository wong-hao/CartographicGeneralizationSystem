...
namespace SMGI.Plugin.EmergencyMap.Common
{
    public class ShapeFileWriter
    {
        ...
        public IFeatureCursor ErrorFeatureCursor
        {
            get { return _errorFeatureCursor; }
            set { _errorFeatureCursor = value; }
        }
        ...
        public bool createErrorResutSHPFile(string fullFileName, ISpatialReference sr, esriGeometryType geoType, Dictionary<string, int> fieldName2Len = null, bool bOverride = true)
        {
            ...
            try
            {
                if (System.IO.File.Exists(fullFileName))
                {
                    ...
                    if (File.Exists(path + "\\" + shpName + ".shp"))
                    {
                        ...
                        (pFeatureClass as ITable).DeleteSearchedRows(null);
                        _errorFeatureClass = pFeatureClass;
                        return true;
            ...
            {
                MessageBox.Show("创建错误记录shp失败：" + ex.Message);
                return false;
            }
        }
