...
namespace SMGI.Common
{
    public class GApplication
    {
        ...
        public WaitOperation SetBusy(string title)
        {
            MainForm.BusyStatus = true;
            bool inited = false;
            ThreadStart start = () =>
        ...
        public IWorkspace GetWorkspacWithSDEConnection(string sdeAddress, string sdeUsername, string sdePassword, string sdeDataBaseName)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("无法连接到数据库服务器，请验证服务器地址、用户名、密码、数据库信息，然后重试：" + ex.Message);
            }
            return null;
        }
        ...
        /// <summary>
        /// 连接远程共享目录，如（net use \\172.16.34.105\SMGICollab /User:172.16.34.105\sde 123456 /PERSISTENT:YES）
        /// </summary>
        /// <param name="ipAddress">远程服务器地址</param>
        /// <param name="folderName">共享文件夹名称</param>
        /// <param name="userName">用户名</param>
        /// <param name="passWord">密码</param>
        /// <returns>非空字符串，表示连接失败，否则成功</returns>
        public string NetShareConnectState(string ipAddress, string folderName, string userName, string passWord)
        {
            string err = "";
