...
{
    public class FeatureEditTool: SMGITool
    {
            ...
            try
            {
                ...
                if (group != null && cls != null)
                {
                    ...
                    else
                    {
                        try
                        {
                            int index = pFeature.Fields.FindField(group.Heading);
                            if (index != -1)
                            {
                                pFeature.set_Value(index, cls.Label);
                            }
                        }
                        catch
                        {
        ...
        private void CreateFeature(IGeometry geo)
        {
            try
            {
                ...
                m_Application.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewAll, null, pExtendEnvelop);
            }
            catch (Exception ex)
            {
