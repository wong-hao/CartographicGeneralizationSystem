...
{
    /// <summary>
    /// 在多比例尺的数据库快速出图中，进行地震波要素及注记Element（存在于Mxd中）
    /// </summary>
    public class CreateEarthQuakeWaveTool : SMGI.Common.SMGITool
    {
        ...
        public override void OnMouseDown(int button, int shift, int x, int y)
        {
            if (button != 1)
                return;
            var frm = new SMGI.Plugin.ThematicChart.FrmEarthToolSet();
            ...
            IPoint geoPoint = GetGeo2000(m_Application.ActiveView, anchorPoint.X, anchorPoint.Y);
            Longitude = new LongitudeOrLatitude();
            Latitude = new LongitudeOrLatitude();
            Longitude.Degree = Math.Floor(geoPoint.X);
            Longitude.Minute = Math.Floor((geoPoint.X - Longitude.Degree) * 60);
            Longitude.Second = Math.Floor(((geoPoint.X - Longitude.Degree) * 60 - Longitude.Minute) * 60);
            Latitude.Degree = Math.Floor(geoPoint.Y);
            Latitude.Minute = Math.Floor((geoPoint.Y - Latitude.Degree) * 60);
            Latitude.Second = Math.Floor(((geoPoint.Y - Latitude.Degree) * 60 - Latitude.Minute) * 60);
    ...
    public struct LongitudeOrLatitude
    {
