...
{
    /// <summary>
    /// 回滚服务器数据库中的所有要素至某一指定版本号，即删除服务器数据库中所有大于指定版本号的要素
    /// 注意：需要管理员权限
    /// </summary>
    public class ServerDBRollbackToVersionCmd : SMGICommand
    {
        public ServerDBRollbackToVersionCmd()
        {
        }
        ...
        public override void OnClick()
        {
            ...
            using (var wo = m_Application.SetBusy())
            {
                ...
                delCount = ds.DeleteFeatureInDatabase(fcQF, tableQF, System.Environment.MachineName, string.Format("数据库版本回滚:【删除版本号大于{0}的要素】", frm.DBVersion));
                if (delCount < 0)
                {
                    res = false;
                }
