...
namespace SMGI.Plugin.EmergencyMap
{
    ...
    public class AnnoBreakFaceToNorth
    {
        ...
        public string BreakfclName
        {
            get;
            set;
        }
        public AnnoBreakFaceToNorth(GApplication app)
        {
            _app = app;
        }

        public string toNorth(Dictionary<string, IFeatureClass> annoName2FeatureClass, WaitOperation wo = null)
        {
            string msg = "";

            if (annoName2FeatureClass == null || annoName2FeatureClass.Count == 0)
                return msg;

            var targetLayer = _app.Workspace.LayerManager.GetLayer(new SMGI.Common.LayerManager.LayerChecker(l =>
            { return l is IGeoFeatureLayer && (l as IFeatureLayer).FeatureClass.AliasName == BreakfclName; })).FirstOrDefault() as IFeatureLayer;
            if (targetLayer == null || targetLayer.FeatureClass == null)
            {
                msg = string.Format("没有找到要素类【{0}】！", BreakfclName);
                System.Diagnostics.Trace.WriteLine(msg);

                return msg;
            }
            var targetFC = targetLayer.FeatureClass;

            foreach (var kv in annoName2FeatureClass)
            {
                IFeatureClass annoFeatureClass = kv.Value;

                if (wo != null)
                    wo.SetText("正在处理" + (annoFeatureClass as IDataset).Name + "……");

                int classID = targetFC.ObjectClassID;
                IQueryFilter qf = new QueryFilterClass { WhereClause = "AnnotationClassID = " + classID };
                IFeatureCursor annoCursor = annoFeatureClass.Search(qf, false);
        ...
        private void SplitAnnoFeature(IFeature annoFeature)
        {
            ...
            if (textElementGeometry is IPolyline)
            {
                ...
                if (pointCollection.PointCount == 2)
                {
                    ...
                    RegularAnnoSplit(annoFeature, pAnnoFeature as IAnnotationFeature2, pTextElement);
                }
                if (pointCollection.PointCount > 2)
                {
                    ...
                    if (pointCollection.PointCount != pTextElement.Text.Length * 2)
                    {
                        ...
                        NonRegularAnnoSplit2(annoFeature);
                    }
                    else
                        NonRegularAnnoSplit(annoFeature);
                }
            }
        }
        ...
        /// <returns></returns>
        private void NonRegularAnnoSplit(IFeature annoFeature) {
            //获取annoFeature的要素图层
            ...
            for (int i = 0; i < lyrs.Length; i++)
            {
                ...
                if (pFeatureLayer != null && (pFeatureLayer.FeatureClass as IDataset).Name == (annoFeature.Class as IDataset).Name)
                {
            ...
            { return  l is IFeatureLayer&& (l as IFeatureLayer).FeatureClass.FeatureClassID == AnnotationClassID; })).FirstOrDefault();
            if (FeatureID == 851)
            {
            }
            var linkfeature = (lyr as IFeatureLayer).FeatureClass.GetFeature(FeatureID);
            var linkgeo = linkfeature.Shape as IPolyline;
            double x = linkgeo.ToPoint.X - linkgeo.FromPoint.X;
            double y = linkgeo.ToPoint.Y - linkgeo.FromPoint.Y;
            #region 计算夹角
            ...
            ITransform2D ptrans = pl as ITransform2D;
            IPoint mid = new PointClass();
            pl.QueryPoint(esriSegmentExtension.esriNoExtension, 0.5, true, mid);
            ptrans.Scale(mid, 1.5, 1.5);
            ...
            for (int i = 0; i < pAnnoText.Length; i++)
            {
                ...
                pNewTextSymbol.Angle = 0;
                double rotateAngle = GetAngle(pNewPolyline, _app.ActiveView.FocusMap.MapScale);
                if ((ss == "（" || ss == "）" || ss == "(" || ss == ")") && Math.Round(rotateAngle, 0) % 360 != 0)
                {     
                    pNewTextSymbol.Angle = rotateAngle;
                }
        ...
        /// <param name="annoFeature"></param>
        /// <returns></returns>
        private void NonRegularAnnoSplit2(IFeature annoFeature)
        {
            //获取annoFeature的要素图层
            IFeatureLayer pAnnoFeatureLayer = null;
            var lyrs = _app.Workspace.LayerManager.GetLayer(new SMGI.Common.LayerManager.LayerChecker(l =>
            ...
            { return (l as IFeatureLayer).FeatureClass.FeatureClassID == AnnotationClassID; })).FirstOrDefault();

            var linkfeature = (lyr as IFeatureLayer).FeatureClass.GetFeature(FeatureID);
            var linkgeo = linkfeature.Shape as IPolyline;
            double x = linkgeo.ToPoint.X - linkgeo.FromPoint.X;
            double y = linkgeo.ToPoint.Y - linkgeo.FromPoint.Y;
            //判断polyline的方向，在45~180的均要进行polyline的转向，保证注记由上往下

            #region 计算夹角
          
            AdjRiverAnno(polyline);
            ...
            for (int i = 0; i < textLength; i++)
            {
                ...
                if (0 < Splitdistance && Splitdistance < polyline.Length)
                {
                    ...
                    if (!s_point.IsEmpty)
                    {
                        ITextSymbol pNewTextSymbol = (pTextElement.Symbol as IClone).Clone() as ITextSymbol;
                        pNewTextSymbol.Angle = 0;
                        double rotateAngle = GetAngle(polyline, _app.ActiveView.FocusMap.MapScale);
                        if ((s == "（" || s == "）" || s == "(" || s == ")") && Math.Round(rotateAngle, 0) % 360 != 0)
                        {      
                            pNewTextSymbol.Angle = rotateAngle;
                        }
                        ITextElement pNewTextElement = new TextElementClass();
        ...
        private void RegularAnnoSplit(IFeature _Feature, IAnnotationFeature2 pAnnoFeature, ITextElement pTextElement)
        {
            ...
            {
                IFeatureLayer pFeatureLayer = lyrs[i] as IFeatureLayer;
                if (pFeatureLayer != null && (pFeatureLayer.FeatureClass as IDataset).Name == (_Feature.Class as IDataset).Name)
                {
                    pAnnoFeatureLayer = pFeatureLayer;
                    break;
                }
            ...
            {
                IPolyline pl1 = pLineGeometries[i] as IPolyline;//注记的宽度
                IPointCollection pc =(pl1 as IClone).Clone() as IPointCollection;
               
          
                IPolyline pl = pc as IPolyline;
                #region 计算夹角
                AdjRiverAnno(pl);
                #endregion
                ...
                for (int j = 0; j < lineText.Length; j++)
                {
                    ...
                    if (0 < Splitdistance && Splitdistance < pl.Length)
                    {
                        ...
                        if (!s_Positon.IsEmpty)//【核心：创建打散要素】
                        {
                            ITextSymbol pNewTextSymbol = (pTextElement.Symbol as IClone).Clone() as ITextSymbol;
                            pNewTextSymbol.Angle = 0;
                            double rotateAngle = GetAngle(pl, _app.ActiveView.FocusMap.MapScale);
                            if ((s == "（" || s == "）" || s == "(" || s == ")") && Math.Round(rotateAngle, 0) % 360 != 0)
                            {   
                                pNewTextSymbol.Angle = rotateAngle;
                            }
        ...
        /// 调整注记排列方向
        /// </summary>
        private void AdjRiverAnno(IPolyline pl)
        {
        ...
        /// <summary>
        /// 折线注记的反向处理，用于注记中，方向在45度~135度的进行反向,保证注记由上往下
        /// </summary>
        /// <param name="textPath"></param>
        public bool ReversePolylineDirection(IPolyline textPath)
        {
            bool reverse = false;
            ...
            double angle = Math.Atan2(dy, dx) * 180 / Math.PI;//弧度转度[-90,+90]
            ILine line = new LineClass();
            line.PutCoords(fromPoint, toPoint);
            angle = line.Angle * 180 / Math.PI;
            if (angle < 0)
                angle = 360 + angle;
            //45度到135度进行折线的反转
        ...
        public double GetAngle(IPolyline pPolyline, double MapScale)
        {
