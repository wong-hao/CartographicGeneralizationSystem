...
{
    public class MapRotateTool : SMGITool
    {
        ...
        private IRepresentation editRep;//当前的旋转的要素
        ...
        public override void OnClick()
        {
            ...
            var reps = m_Application.Workspace.SelectedRepresentations;
            editRep = reps[0];
            IEnumFeature pEnumFeature = (IEnumFeature)pMap.FeatureSelection;
            ((IEnumFeatureSetup)pEnumFeature).AllFields = true;
            m_RotateFeature = editRep.Feature;
            m_BasePoint = ((IArea)editRep.Shape.Envelope).Centroid;
                ...
                else
                {
                    ...
                    if (!double.IsNaN(m_RotTrack.Angle))
                    {
                        ...
                        else
                        {
                            ...
                            pTrans2D = (ITransform2D)editRep.ShapeCopy;
                            pTrans2D.Rotate(m_RotTrack.Origin, m_RotTrack.Angle);
                            editRep.Shape = (IGeometry)pTrans2D;
                            editRep.UpdateFeature();
                            m_RotateFeature.Store();
                        }
