    ...
    public static class HydroAlgorithm
    {
        ...
        public class HydroGraph
        {
            ...
            public List<HydroGraph> BuildTrees()
            {
                ...
                {
                    //构建树
                    trees[edge.TreeIndex - 1].Edges.Add(edge);

                    //下游点
                    var toPoint = new PointWarp(edge.To.Point);
                    if (!trees[edge.TreeIndex - 1].Nodes.ContainsKey(toPoint))
                    {
                        trees[edge.TreeIndex - 1].Nodes.Add(toPoint, edge.To);
                    }
                    //上游点
                    var fromPoint = new PointWarp(edge.From.Point);
                    if (!trees[edge.TreeIndex - 1].Nodes.ContainsKey(fromPoint))
                    {
                        trees[edge.TreeIndex - 1].Nodes.Add(fromPoint, edge.From);
                    }
