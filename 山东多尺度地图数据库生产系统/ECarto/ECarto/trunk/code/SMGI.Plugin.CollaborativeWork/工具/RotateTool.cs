...
{
    public  class RotateTool: SMGITool
    {
        public RotateTool()
        {
        ...
        public override void OnMouseDown(int button, int shift, int x, int y)
        {
            if (button == 1 && m_RotateFeature != null)
            {
                ...
                else
                {
                    ...
                    if (pGeo.GeometryType == esriGeometryType.esriGeometryPoint)//201605
                    {
                        IFeatureClass pFeatureClass = m_RotateFeature.Class as IFeatureClass;
                        var lyrs = m_Application.Workspace.LayerManager.GetLayer(new LayerManager.LayerChecker(l =>
                        {
                            return l is IGeoFeatureLayer && l.Visible && (l as IGeoFeatureLayer).Selectable;
                        })).ToArray();
                        IFeatureLayer pFeatureLayer = null;
                        for (int i = 0; i < lyrs.Length; i++)
                        {
                            IFeatureLayer _FLyer = lyrs[i] as IFeatureLayer;
                            if (_FLyer.FeatureClass.AliasName == pFeatureClass.AliasName)
                            {
                                pFeatureLayer = _FLyer;
                                break;
                            }
                        }
                        IFeatureRenderer _FRender = (pFeatureLayer as IGeoFeatureLayer).Renderer;
                        ISymbol _Symbol = _FRender.get_SymbolByFeature(m_RotateFeature);

                        m_RotTrack.AddPoint(pGeo, _Symbol as IMarkerSymbol);
                    }
                    else
                    {
                     ...
                     if (!double.IsNaN(m_RotTrack.Angle))
                     {
                         ...
                         if (m_RotateFeature.Shape.GeometryType == esriGeometryType.esriGeometryPoint)
                         {
                             ...
                             if (ai != -1 && m_RotateFeature.Value[ai] != null)
                             {
                                 ...
                                 try
                                 {
                                     angle = Convert.ToDouble(m_RotateFeature.Value[ai]);
                                 }
                                 catch
                                 {
                                     angle = 0;
