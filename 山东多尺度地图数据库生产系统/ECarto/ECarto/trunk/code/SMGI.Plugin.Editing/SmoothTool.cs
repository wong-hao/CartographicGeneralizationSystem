...
{
    public class SmoothTool:SMGI.Common.SMGITool
    {
        ...
        /// <summary>
        /// ArcGIS高级编辑工具SmoothTool（平滑工具）
        /// </summary>
        public SmoothTool()
        {
            m_caption = "平滑Tool";
            m_category = "编辑工具";
            m_toolTip = "将要素的直角边和拐角边处理为贝塞尔曲线,右键设置容差";
        }
                ...
                if (geo != null)
                {
                    ...
                    {                        
                        ...
                        if (feature.Shape is IPolyline || feature.Shape is IPolygon)
                        {
                            ...
                            {
                                path.Smooth(maxAllowableOffset * 0.000009);
                                path.Generalize(0.1 * 0.000009);
                            }
                            else
                            {
                                path.Smooth(maxAllowableOffset);
                                path.Generalize(0.1);
                            }
                    ...
                    //m_Application.EngineEditor.StopOperation("平滑Tool操作");
