...
namespace SMGI.Plugin.EmergencyMap
{
    public class EnvironmentSettings
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<string> getMapScaleFromExpertiseConfig()
        {
            List<string> result = new List<string>();

            string fileName = GApplication.Application.Template.Root + @"\专家库\Expertise.xml";
            if (File.Exists(fileName))
            {
                try
                {
                    ...
                    foreach (XElement ele in scaleItems)
                    {
                        int scale = int.Parse(ele.Element("Scale").Value);
