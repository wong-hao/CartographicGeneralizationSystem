    ...
    public class CheckSymbolConflictCmd : SMGICommand
    {
        ...
        public override void OnClick()
        {
            ...
            if (err == "")
            {
                if (File.Exists(outputFileName))
                {
                    ...
                    {
                        IFeatureClass errFC = CommonMethods.OpenSHPFile(outputFileName);
                        CommonMethods.AddTempLayerToMap(m_Application.Workspace.LayerManager.Map, errFC);
                    }
                }
                else
                {
        ...
        public static string DoCheck(string resultSHPFileName, IFeatureLayer objFeatureLayer, IFeatureLayer connFeatureLayer, double refScale, WaitOperation wo = null)
        {
            ...
            try
            {
                ...
                CommonMethods.DeleteShapeFile(resultSHPFileName);
                if (tempFC != null && tempFC.FeatureCount(null) > 0)
                {
                    if (wo != null)
                        wo.SetText(string.Format("正在导出检查结果......"));
                    CommonMethods.ExportFeatureClassToShapefile(tempFC, resultSHPFileName);
                }
