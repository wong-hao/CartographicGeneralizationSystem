...
{
    public partial class PlaceNameLocationForm : Form
    {
        GApplication _app;
        ...
        string _placeNameDatabase = "dcdmeta";
        string _placeNameLayerName = "AGNP";
        string _placeNameFieldName = "NAME";
        ...
        public IPoint CenterPoint
        {
            ...
            protected set
            {
                ...
                {
                    if (m_centerPoint.SpatialReference == null)
                    {
                        m_centerPoint.SpatialReference = _app.MapControl.Map.SpatialReference;
                    }
                    else if (m_centerPoint.SpatialReference.Name != _app.MapControl.Map.SpatialReference.Name)
                    {
                        (m_centerPoint as IGeometry).Project(_app.MapControl.Map.SpatialReference);//对裁切中心点进行投影变换
                    }

                    IEnvelope ext = _app.MapControl.Extent;
                    ext.CenterAt(m_centerPoint);

                    _app.MapControl.Extent = ext;
                    _app.MapControl.ActiveView.ScreenDisplay.UpdateWindow();
                    _app.MapControl.FlashShape(m_centerPoint);

                    updateClipGroupElement();
                }
            }
        }
        ...
        public PlaceNameLocationForm(GApplication app)
        {
            InitializeComponent();
        ...
        private void btnResearch_Click(object sender, EventArgs e)
        {
            ...
            IWorkspace sdeWorkspace = _app.GetWorkspacWithSDEConnection(_sdeAddress, _userName, _password, _placeNameDatabase);
            ...
            if (!(sdeWorkspace as IWorkspace2).get_NameExists(esriDatasetType.esriDTFeatureClass, _placeNameLayerName))
            {
                MessageBox.Show("无法访问服务器中的地名库！");

                return;
            }
            IFeatureWorkspace sdeFeatureWorkspace = sdeWorkspace as IFeatureWorkspace;
            IFeatureClass placeNameFeatureClass = sdeFeatureWorkspace.OpenFeatureClass(_placeNameLayerName);
            if (placeNameFeatureClass == null)
            {
                return;
            }

            search(placeNameFeatureClass, _placeNameFieldName, tbPlaceName.Text);
        ...
        {
            ClipElement.SetClipGroupElement(_app, _oldClipGroupElement);
        ...
        /// 根据地名模糊搜索
        /// </summary>
        /// <param name="placeNameFeatureClass">地名要素类（AGNP）</param>
        /// <param name="fieldName">字段（NAME）</param>
        /// <param name="keyVal">关键字</param>
        ...
        private void updateResultTable()
        {
            ...
            if (_result != null)
            {
                for (int i = 0; i < _result.Count; ++i)
                {
                    ...
                    if(f == null || f.Shape == null || f.Shape.GeometryType != esriGeometryType.esriGeometryPoint)
                    {
                        continue;
                    }
                    int indexOfName = f.Fields.FindField(_placeNameFieldName);
                    if(indexOfName == -1)
                    {
                        continue;
                    }
            ...
            {
                int index = int.Parse(dgResult.CurrentRow.HeaderCell.Value.ToString());
        ...
        private void updateClipGroupElement()
        {
            #region 自动计算空间参考
            ...
            #endregion
            //更新裁切框
            IGroupElement clipELe = ClipElement.createClipGroupElement(_app, CenterPoint, ClipPaperSize.PaperWidth, ClipPaperSize.PaperHeight, RefScale);
            ClipElement.SetClipGroupElement(_app, clipELe);
        }
        #endregion

        private void PlaceNameLocationForm_Load(object sender, EventArgs e)
        {
            var envFileName = @"专家库\数据定位\LocationSearch.xml";
          ...
          
            {
                ...
                int i = 0;
                _placeNameDatabase = server.Element("DataBase").Value;
                _placeNameLayerName = server.Element("Lyr").Value;
                _placeNameFieldName = server.Element("Field").Value;

                _sdeAddress = server.Element("IPAddress").Value;
