...
namespace SMGI.Plugin.MapGeneralization
{
    ...
    /// 沟渠分级：end
    /// </summary>
    public class CanalGradualCmd :SMGICommand
    {
        ...
        {
            hydlLayer = GetLayer();
            FrmCanalGradeSet frm = new FrmCanalGradeSet();
        ...
        public void DoDitchGrade()
        {
            ...
            var range = (fc as IGeoDataset).Extent;
            if (range == null)
                return;
            PlanarGraph.SELECTAREA = 1350000;
            //PlanarGraph.DANGLELENGTH *= 10;
            //PlanarGraph.DANGLENMAX *= 10;
            ...
            using (WaitOperation wo = m_Application.SetBusy())
            {
                ...
                foreach (var face in pg.Faces)
                {
                    ...
                    if (face.Area > 0 && face.Area < PlanarGraph.SELECTAREA)
                    {
                        ...
                        //if (!pg.StrokeDic.ContainsKey(curEdge.StrokeID))
                        //    continue;
                        do
                        {
                ...
                wo.SetText("8.绘制中删除沟渠");
                IGraphicsContainer gh = m_Application.ActiveView as IGraphicsContainer;
                gh.DeleteAllElements();
                foreach (var oid in deleteFeatures.Keys)
                {
                    IFeature fe1 = fc.GetFeature(oid);
                    LineElementClass line = new LineElementClass();
                    line.Geometry = fe1.ShapeCopy;
                   
                    gh.AddElement(line, 0);
