...
{
    public class CutForDitches : BaseGenTool
    {
        ...
        public CutForDitches()
        {
            base.m_category = "GWater";
            base.m_caption = "分离并选取沟渠";
            base.m_message = "分离沟渠";
            base.m_toolTip = "对沟渠进行面分离，面中轴化，线分离，合并，隔条选取等";
            base.m_name = "ditches";
            waterLayer = null;
        ...
        public override void OnDblClick()
        {
            ...
            fb = null;
            //Gen(p);
            if (System.Windows.Forms.MessageBox.Show("将进行沟渠分离、中轴化、选取等操作，请确认", "提示", System.Windows.Forms.MessageBoxButtons.OKCancel, System.Windows.Forms.MessageBoxIcon.Warning) == System.Windows.Forms.DialogResult.Cancel)
            {
                return;
            }
            ...
            centralizeAndReshape(waterLayer);
            ...
            if (WS2.get_NameExists(esriDatasetType.esriDTFeatureClass, "沟渠中心线"))
            {
                ...
                ReshapeDitchLine2(centralizedDitch2,p);
        ...
        public override void OnClick()
        {
            ...
            //{
            //    IFeatureClass centralizedDitch2 = FeatWS.OpenFeatureClass("沟渠中心线");
            //    ReshapeDitchLine(centralizedDitch2);
            //    connectDitchLine(centralizedDitch2);
            //    unionTouchedDitchLines2(centralizedDitch2);
        ...
        void centralizeAndReshape(GLayerInfo orgLayerInfo)
        {
            IFeatureLayer waterLayer = orgLayerInfo.Layer as IFeatureLayer;
            IFeatureClass fc = waterLayer.FeatureClass;
            ...
            try
            {
                ...
                while ((feat = fCur.NextFeature()) != null)
                {
                    ...
                    try
                    {
                        ...
                        IPolygon forMinArea = geoCopy as IPolygon;
                        double width = ((forMinArea as IArea).Area * 2.5) / forMinArea.Length;
                        if (width > (double)m_application.GenPara["水系中轴化宽度阈值"])//10
                        {
                        ...
                        //forInsertBuffer.set_Value(_GenUsed2, -1);
        ...
        void ReshapeDitchLine(IFeatureClass DitchLineFeats)
        {
            ...
            {
                WaitOperation w = m_application.SetBusy(true);
                int cou = DitchLineFeats.FeatureCount(null);
                IFeatureCursor fCur = DitchLineFeats.Update(null, false);
                ...
                while ((feat = fCur.NextFeature()) != null)
                {
                    ...
                    //for (int k = 0; k < pathColl.GeometryCount; k++)
                    //{
                    //    IPath tpPath = pathColl.get_Geometry(k) as IPath;
                    //    if (tpPath.Length < 10)
                    //    {
                    //        continue;
                    //    }
                    //    IFeatureBuffer tpBuffer = feat as IFeatureBuffer;
                    //    IPolyline tpLine = new PolylineClass();
                    //    IGeometry tpGeo = tpPath as IGeometry;
                    //    (tpLine as IGeometryCollection).AddGeometries(1, ref tpGeo);
                    //    tpBuffer.Shape = tpLine;
                    //    int insertID = Convert.ToInt32(insertCur.InsertFeature(tpBuffer));
                    //    isInsertedFeat.Add(insertID, true);
                    //}

                    ISegmentCollection segColl = line as ISegmentCollection;
                    for (int m = 0; m < segColl.SegmentCount; m++)
                    {
                        ISegment seg = segColl.get_Segment(m);
                        if (seg.Length < 1)
                        {
                            continue;
                        }
                        double angleForSeg=(seg.FromPoint.Y-seg.ToPoint.Y)/(seg.FromPoint.X-seg.ToPoint.X);
                        index_angle.Add(m, angleForSeg);
                    }
                    foreach (int index in index_angle.Keys)
                    {
                        if (isgrouped.Contains(index))
                        {
                            continue;
                        }
                        IPolyline newLine = new PolylineClass();
                        //ISegmentCollection newColl = newLine as ISegmentCollection;
                        IGeometryCollection newColl = newLine as IGeometryCollection;

                        ISegment oris = segColl.get_Segment(index);
                        IPath oriPath = new PathClass();
                        (oriPath as ISegmentCollection).AddSegments(1, ref oris);
                        IGeometry newo = oriPath as IGeometry;
                        newColl.AddGeometries(1, ref newo);
                        isgrouped.Add(index);

                        double Angle = index_angle[index];
                        foreach (int toIndex in index_angle.Keys)
                        {
                            if (toIndex != index&&!isgrouped.Contains(toIndex))
                            {
                                double toAngle = index_angle[toIndex];
                                if (Math.Abs(Angle - toAngle) < 0.5)
                                {
                                    ISegment s = segColl.get_Segment(toIndex);
                                    //newColl.AddSegments(1, ref s);
                                    IPath newPath = new PathClass();
                                    (newPath as ISegmentCollection).AddSegments(1, ref s);
                                    IGeometry newn = newPath as IGeometry;
                                    newColl.AddGeometries(1, ref newn);
                                    isgrouped.Add(toIndex);
                                }
                            }
                        }
                        IGeometryCollection pathColl2 = newLine as IGeometryCollection;
                        for (int k = 0; k < pathColl2.GeometryCount; k++)
                        {
                            IPath tpPath = pathColl2.get_Geometry(k) as IPath;
                            if (tpPath.Length < 8.2)//向后面的挂接函数中的dis参数看齐
                            {
                                continue;
                            }
                            IFeatureBuffer tpBuffer = feat as IFeatureBuffer;
        ...
        void connectDitchLine(IFeatureClass DitchLineFeats)
        {
            try
            {
                ...
                {
                    w.SetText("4/11:正在进行分析准备:" + feature.OID.ToString());
                    w.Step(featureCount);
                ...
                {
                    w.SetText("5/11:正在分析:" + i.ToString());
        ...
        void AutoProcess(int index, bool commit,IFeatureClass DitchClass)
        {
            if (commit)
            {
                ...
                foreach (var item in group.Keys)
                {
                    ...
                    {
                        //if (ID_groupIDs.ContainsKey(fid))
                        //{
                        //    List<int> groupIDs = ID_groupIDs[fid];
                        //    groupIDs.AddRange(fids);
                        //    groupIDs.AddRange(group.Keys);
                        //}
                        //else
                        //{
        ...
        //        if (hit.HitTest(p, sDis, esriGeometryHitPartType.esriGeometryPartBoundary | esriGeometryHitPartType.esriGeometryPartCentroid, hitPoint, ref distance, ref  part, ref seg, ref rightSide))
        //        {
        //            try
        //            {
