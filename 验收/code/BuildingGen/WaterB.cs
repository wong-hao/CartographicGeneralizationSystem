...
{
    public class WaterB : BaseGenTool
    {
        ...
        public WaterB()
        {
            ...
            {
                new GenDefaultPara("水系融合距离",(double)25)
                //,new GenDefaultPara("水系最小上图面积",(double)100)
                ,new GenDefaultPara("水系融合_最小保留洞面积",(double)800)
            };
        ...
        private void GroupLayer()
        {
            ...
            try
            {
                ...
                {
                    wo.Step(wCount);
                    ITopologicalOperator tooo = feature.ShapeCopy as ITopologicalOperator;
                    (tooo as IPolycurve).Generalize(0.001);
                    tooo.Simplify();
                    feature.Shape = tooo as IGeometry;
                    feature.Store();
                ...
                ap.aggregation_distance = m_application.GenPara["水系融合距离"];
                //ap.minimum_area = m_application.GenPara["建筑物最小上图面积"];
                ap.minimum_hole_size = m_application.GenPara["水系融合_最小保留洞面积"];
                ...
                double miniArea = (double)m_application.GenPara["水系最小上图面积"];
        ...
        private void GroupLayer2()
        {
            ...
            try
            {
                ...
                wo.SetText("正在进行分组合并操作。");
                IPoint centerPoint = (mp.Envelope as IArea).Centroid;
                ITransform2D mp_scaled = mp.Clone() as ITransform2D;
                mp_scaled.Scale(centerPoint, 20, 20);
                IPointCollection mpc_scaled = mp_scaled as IPointCollection;
                for (int i = 0; i < mp.PointCount; i++)
                {
                    IPoint bp = mp.get_Point(i);
                ...
                ap.out_feature_class = m_application.Workspace.LayerManager.TempLayerName();
                ap.aggregation_distance = m_application.GenPara["水系融合距离"];
                //ap.minimum_area = m_application.GenPara["建筑物最小上图面积"];
                ap.minimum_hole_size = m_application.GenPara["水系融合_最小保留洞面积"];
                //ap.aggregation_distance = 22;
                ap.minimum_area = 500;
                //ap.minimum_hole_size = 800;
                ap.orthogonality_option = "NON_ORTHOGONAL";

                object a=m_application.Geoprosessor.Execute(ap, null);
                if (a == null)
                {
