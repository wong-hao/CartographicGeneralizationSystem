...
{
    public class WaterD : BaseGenTool
    {
        INewPolygonFeedback fb;
        GLayerInfo buildingLayer;
        IFeatureClass roadLayer;
        int fieldID;
        int maxValue;
        ...
        public WaterD()
        {
            ...
            base.m_caption = "自动分组";
            base.m_message = "湖泊自动分组";
            base.m_toolTip = "绘制多边形选择湖泊区域，\n由计算机对湖泊自动分组。";
            base.m_name = "WaterD";
            base.m_usedParas = new GenDefaultPara[] 
            {
                new GenDefaultPara("湖泊自动分组面积",(double)20000),
                new GenDefaultPara("湖泊自动分组距离",(double)100)
            };
        ...
        private bool isDitchTri(ITinTriangle tri)
        {
            for (int k = 0; k < 3; k++)
            {
                ITinNode node = tri.get_Node(0);
                if (node.TagValue == -1)
                {
                    return true;
                }
        ...
        Dictionary<int, IFeature> DitchID_IFeat = new Dictionary<int, IFeature>();
        private void Gen2(IPolygon range)
        {
            ...
            if (roadLayer != null)
            {
                ...
                roadCursor = roadLayer.Search(sf, true);
                allCount += roadLayer.FeatureCount(sf);
            }  
            ...
            try
            {
                ...
                while ((feature = fCursor.NextFeature()) != null)
                {
                    if ((feature.Shape as IArea).Area > 100000000)
                    {
                        continue;
                    }
                    aStep(allCount);
                    ...
                    if (Convert.ToString(feature.get_Value(YSDMid)) == "632250")
                    {
                        DitchID_IFeat.Add(feature.OID, feature);
                    }
                    for (int i = 0; i < (geo as IPointCollection).PointCount; i++)
                    {
                        IPoint p = (geo as IPointCollection).get_Point(i);
                        p.Z = 0;
                        if (Convert.ToString(feature.get_Value(YSDMid)) == "632250" || Convert.ToString(feature.get_Value(YSDMid)) == "614450")
                        {
                            tin.AddPointZ(p, -1);
                            continue;
                        }
                ...
                for (int i = 1; i <= tin.TriangleCount; i++)
                {
                    ...
                    if (!triangle.IsInsideDataArea||isDitchTri(triangle))
                    {
                        continue;
                    }
                ...
                foreach (var item in results)
                {
                    ...
                    foreach (var oid in item.OIDs)
                    {
                        ...
                        if (Convert.ToString(fe.get_Value(YSDMid)) == "632250" || Convert.ToString(fe.get_Value(YSDMid)) == "614450")
                        {
                            continue;
                        }
                        fe.set_Value(fieldID, maxValue);
        ...
        private IFeatureClass GetRoadLayer(GWorkspace workspace)
        {
            IFeatureClass centralizedDitch = null;
            IFeatureWorkspace FeatWS = workspace.Workspace as IFeatureWorkspace;
        ...
        {
            return area > (double)m_application.GenPara["湖泊自动分组面积"];
        }
        /// <summary>
        /// 判断距离与 建筑物自动分组距离的关系
        /// </summary>
        ...
        {
            double l = (double)m_application.GenPara["湖泊自动分组距离"];
        ...
        //private double calculateAngleOfLine(IPolyline line)
        //{
        //    line.Generalize(1);
            
        //}

        private void connectDitches(IPolygon range)
        {
            IFeatureLayer newLayer = new FeatureLayerClass();
            newLayer.FeatureClass = (buildingLayer.Layer as IFeatureLayer).FeatureClass;
            IFeatureClass newFC = newLayer.FeatureClass;
            newLayer.Name = "ywh";
            ISpatialFilter sf = new SpatialFilterClass();
            sf.Geometry = range;
            sf.SpatialRel = esriSpatialRelEnum.esriSpatialRelIntersects;
            sf.WhereClause = "要素代码='632250'";
            IFeatureSelection fselect = newLayer as IFeatureSelection;
            ...
            foreach (IFeature tpF in DitchID_IFeat.Values)
            {
                IPolygon tpPolygon = tpF.ShapeCopy as IPolygon;
                BuildingGenCore.CenterLine cl = cf.Create2(tpPolygon);
                IPolyline tpPolyline = cl.Line;
                tpPolyline.Generalize(1);
